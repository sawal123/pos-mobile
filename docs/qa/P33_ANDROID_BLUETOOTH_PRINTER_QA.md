...
| Check | Result | Detail |
|---|---|---|
| `npm run test:unit -- --run` | BASELINE FAILURE | 35/36 files, 1069/1070 tests PASS. Only `business-template.spec.js` fails. |
| P33 targeted suites | PASS | 120/120 across ESC/POS builder, printer service, settings/store/persistence, receipt view printing, and SQLite persistence. |
| `npm run build` | PASS | Vite production build completed. |
| `npx cap sync android` | PASS | SQLite, App, Network, Secure Storage plugins detected. |
| `android/gradlew.bat assembleDebug` | PASS | `BUILD SUCCESSFUL`; APK 13,477,149 bytes at `android/app/build/outputs/apk/debug/app-debug.apk`. |
| Java compilation | PASS | `compileDebugJavaWithJavac` succeeded after two compile fixes (holder iteration type, checked-exception narrowing). |
...
| Android | 16 (SDK 36) |
| Install | `adb install -r` (non-destructive; no uninstall, no `pm clear`, no DB wipe) |
...
Filtered `AndroidRuntime`, `Capacitor`, `BluetoothPrinter`, `BluetoothSocket`, `chromium` after install/launch: no FATAL EXCEPTION, no Bluetooth `SecurityException`, no socket-leak or "socket might closed" noise, no ANR, and no main-thread blocking report. The only Chromium entry observed in earlier sessions was an unrelated sync DNS error from a browser process, not from the app.
